...
+-------------------+     +-------------------+     +-------------------+
|   用户交互层       |     |   前端可视化层     |     |   数据与知识层     |
|  (自然语言输入)    | |<--->| (Web UI / 运行图)  | |<--->| (真实时刻表/RAG)  |
+-------------------+     +-------------------+     +-------------------+
           |                         |                        |
           v                         v                        v
+-------------------+     +-------------------+     +-------------------+
|   L1 数据建模层    | --> |   L2 策略规划层    | --> |   L3 求解执行层    |
| (AccidentCard提取) |     | (求解器选择/参数)  |     | (多算法调度引擎)   |
+-------------------+     +-------------------+     +-------------------+
           |                                              |
           v                                              v
+-------------------+     +-------------------+     +-------------------+
|   L4 方案评估层    | |<--- |   约束验证与规则   | |<--- |   多算法对比框架   |
| (指标计算/LLM评估) |     |   (Headway/区间)   |     | (评分/排序/推荐)   |
+-------------------+     +-------------------+     +-------------------+
```
...
- 支持5种对比准则的动态权重调整
- 根据场景类型自动推荐最优算法
- 兼顾实时性（FCFS毫秒级）与最优性（MIP）

### 6.4 完整的验证体系
- 三层约束验证（时间单调性、追踪间隔、区间运行时间）
- 多维度评估指标（延误、传播、均衡、恢复）
- 综合评级机制（A/B/C/D）
