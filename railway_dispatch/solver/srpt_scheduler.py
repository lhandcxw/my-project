# -*- coding: utf-8 -*-
"""
铁路调度系统 - SRPT（最短剩余处理时间优先）调度器模块
实现列车调度的最短剩余处理时间优先策略

核心思想：
1. 基于当前时刻计算每列车的剩余运行时间
2. 优先调度剩余时间短的列车，快速释放系统容量
3. 适合实时延误调整和局部中断场景
"""
...
class SRPTScheduler:
    """
    最短剩余处理时间优先调度器（Shortest Remaining Processing Time）

    真实高铁调度策略：
    1. 基于当前时刻计算每列车的剩余运行时间（从当前位置到终点）
    2. 优先调度剩余时间短的列车，快速释放系统容量
    3. 受影响列车保持其延误，并向后传播
    4. 后续列车需要等待，以满足最小追踪间隔
    5. 适合实时延误调整和局部中断场景
    """
    ...
    def _calculate_remaining_time(self, train: Train, schedule: Dict, current_station: str) -> int:
        """
        计算列车从当前位置到终点的剩余运行时间
        ...
        Args:
            train: 列车对象
            schedule: 当前调度时刻表
            current_station: 当前车站

        Returns:
            int: 剩余运行时间（秒）
        """
        train_stations = self._get_stations_for_train(train)
        if current_station not in train_stations:
            return 0

        # 找到当前车站的位置
        try:
            current_idx = train_stations.index(current_station)
        except ValueError:
            return 0

        # 计算从当前站之后的所有区间运行时间
        remaining_time = 0
        for i in range(current_idx, len(train_stations) - 1):
            from_station = train_stations[i]
            to_station = train_stations[i + 1]
            # 使用最小运行时间（考虑延误场景下的追赶）
            remaining_time += self._get_min_section_time(from_station, to_station)
    ...
    def solve(self, delay_injection: DelayInjection, objective: str = "min_max_delay") -> SolveResult:
        ...
        # Step 3: SRPT调度 - 按车站处理追踪间隔，优先处理剩余时间短的列车
        for station in self.stations:
            ...
            # 跳过线路所（股道数为0表示不能停站）
            if track_count == 0:
                logger.debug(f"[SRPT] 跳过线路所 {station_code}，不进行追踪间隔处理")
                continue
                    ...
                    for stop in train.schedule.stops:
                        if stop.station_code == station_code:
                            ...
                            trains_at_station.append({
                                ...
                                'remaining_time': remaining_time  # SRPT关键指标
                            })
                            break

            # SRPT核心：按剩余时间排序（剩余时间短的优先）
            trains_at_station.sort(key=lambda x: x['remaining_time'])
            ...
            for idx, train_info in enumerate(trains_at_station):
                ...
                if delay_needed > 0:
                    ...
                    stations_for_train = self._get_stations_for_train(train)
                    try:
                        idx_station = stations_for_train.index(station_code)
                        for i in range(idx_station, len(stations_for_train)):
                            sc = stations_for_train[i]
                            arr, dep = schedule[(train_id, sc)]
        ...
        return SolveResult(
            ...
            message="SRPT调度成功（最短剩余时间优先）"
        )


def create_srpt_scheduler(trains: List[Train], stations: List[Station]) -> SRPTScheduler:
    """
    创建SRPT调度器实例
    ...
    Returns:
        SRPTScheduler: SRPT调度器实例
    """
    return SRPTScheduler(trains, stations)
