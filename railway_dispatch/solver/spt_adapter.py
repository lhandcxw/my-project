...
class SPTSolverAdapter(BaseSolver):
    """
    SPT 求解器适配器（Shortest Processing Time）
    包装 SPTScheduler 为统一接口
    """
    ...
    def _ensure_scheduler(self, trains: List, stations: List):
        ...
        if self._scheduler is None:
            ...
            self._scheduler = SPTScheduler(train_objs, station_objs)
    ...
    def solve(self, request: SolverRequest) -> SolverResponse:
        ...
        try:
            ...
            logger.exception(f"SPT solver error: {e}")
            return SolverResponse(
                success=False,
                status="solver_failed",
                message=f"SPT求解失败: {str(e)}",
                solver_type="spt",
                error=str(e)
            )
