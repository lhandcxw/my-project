# -*- coding: utf-8 -*-
"""
铁路调度系统 - SPT（最短处理时间优先）调度器模块
实现列车调度的最短处理时间优先策略

核心思想：
1. 基于列车总运行时间（从起点到终点）进行排序
2. 优先调度运行时间短的列车（停站少、运行距离短）
3. 可以快速完成一些短途列车，释放系统容量
4. 适合局部延误场景和短途列车优先的场景
"""
...
class SPTScheduler:
    """
    最短处理时间优先调度器（Shortest Processing Time）
    ...
    def __init__(
        ...
        # 预计算每列车的总运行时间（SPT核心）
        self.train_total_times = self._calculate_train_total_times()
    ...
    def _calculate_train_total_times(self) -> Dict[str, int]:
        """
        预计算每列车的总运行时间（SPT核心）

        Returns:
            Dict[str, int]: {train_id: total_time_seconds}
        """
        train_times = {}
        for train in self.trains:
            train_stations = self._get_stations_for_train(train)
            if len(train_stations) < 2:
                train_times[train.train_id] = 0
                continue
            ...
            for i in range(len(train_stations) - 1):
                ...
                # 使用最小运行时间（考虑延误场景）
                total_time += self._get_min_section_time(from_station, to_station)

            train_times[train.train_id] = total_time
    ...
    def solve(self, delay_injection: DelayInjection, objective: str = "min_max_delay") -> SolveResult:
        ...
        # Step 3: SPT调度 - 按车站处理追踪间隔，优先处理总运行时间短的列车
        for station in self.stations:
            ...
            # 跳过线路所（股道数为0表示不能停站）
            if track_count == 0:
                logger.debug(f"[SPT] 跳过线路所 {station_code}，不进行追踪间隔处理")
                continue
            ...
            for train in self.trains:
                if station_code in self._get_stations_for_train(train):
                    for stop in train.schedule.stops:
                        if stop.station_code == station_code:
                            # 获取总运行时间（SPT核心指标）
                            total_time = self.train_total_times.get(train.train_id, 0)
                            trains_at_station.append({
                                ...
                                'total_time': total_time  # SPT关键指标
                            })
                            break

            # SPT核心：按总运行时间排序（运行时间短的优先）
            trains_at_station.sort(key=lambda x: x['total_time'])
        ...
        return SolveResult(
            ...
            message="SPT调度成功（最短处理时间优先）"
        )


def create_spt_scheduler(trains: List[Train], stations: List[Station]) -> SPTScheduler:
    """
    创建SPT调度器实例
    ...
    Returns:
        SPTScheduler: SPT调度器实例
    """
    return SPTScheduler(trains, stations)
