# -*- coding: utf-8 -*-
"""
MaxDelayFirst 求解器适配器模块
将 MaxDelayFirst 调度器包装为统一接口
"""
...
class MaxDelayFirstSolverAdapter(BaseSolver):
    """
    MaxDelayFirst 求解器适配器
    包装 MaxDelayFirstScheduler 为统一接口
    """

    def __init__(self):
        """初始化 MaxDelayFirst 适配器"""
        self._scheduler = None

    def _ensure_scheduler(self, trains: List, stations: List):
        """确保调度器已初始化"""
        if self._scheduler is None:
            # 导入并创建 MaxDelayFirst 调度器
            ...
            self._scheduler = MaxDelayFirstScheduler(train_objs, station_objs)
    ...
    def solve(self, request: SolverRequest) -> SolverResponse:
        ...
        try:
            ...
            # 执行求解
            result = self._scheduler.solve(delay_injection, request.objective)
            ...
            logger.exception(f"MaxDelayFirst solver error: {e}")
            return SolverResponse(
                success=False,
                status="solver_failed",
                message=f"MaxDelayFirst求解失败: {str(e)}",
                solver_type="max_delay_first",
                error=str(e)
            )

    def get_solver_type(self) -> str:
        return "max_delay_first"