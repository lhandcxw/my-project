...
class NoOpSolverAdapter(BaseSolver):
    """
    NoOp 求解器适配器
    包装 NoOpScheduler 为统一接口（基线调度器）
    """
    ...
    def _ensure_scheduler(self, trains: List, stations: List):
        ...
        if self._scheduler is None:
            ...
            self._scheduler = NoOpScheduler(train_objs, station_objs)
    ...
    def solve(self, request: SolverRequest) -> SolverResponse:
        ...
        try:
            ...
            logger.exception(f"NoOp solver error: {e}")
            return SolverResponse(
                success=False,
                status="solver_failed",
                message=f"NoOp求解失败: {str(e)}",
                solver_type="noop",
                error=str(e)
            )
