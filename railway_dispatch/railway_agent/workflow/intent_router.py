...
1. **dispatch（调度处理）**
   用户描述了需要调度系统进行处置的场景，包括列车故障、延误、限速、封锁、施工等需要生成调度方案的情形。
   示例："G1563在石家庄站因大风限速，预计延误15分钟"、"保定东站设备故障，请给出处置建议"

2. **query（信息查询）**
   用户希望获取客观数据或实时状态，如列车时刻表、列车当前运行位置、车站负荷统计等。
   示例："查询G1563的时刻表"、"G1565目前运行状态"、"石家庄站当前列车密度"

3. **chat（知识问答/对话）**
   用户咨询高铁运营相关的专业知识、规章制度，或进行一般性交流。
   示例："高铁追踪间隔的技术标准是多少"、"什么是区间封锁"、"你好"
...
约束条件：
- query_type 仅在 intent=query 时填写有效值，其余情况统一填 unknown
- entities 中缺失的字段必须置为 null，不得省略字段
- 仅输出JSON格式内容，禁止添加解释性文字或代码块标记"""
