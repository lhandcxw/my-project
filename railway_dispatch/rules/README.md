...
**注意**：`avg_delay` 在系统评估中取"各受影响列车最大延误的平均值"，而非所有站点延误的平均值。
...
> 参数来源：`config/dispatch_env.yaml` 中的 `headway_time`
...
| 参数 | 默认值 | 说明 |
|------|--------|------|
| stop_time_redundancy_ratio | 0.8 | 停站时间冗余系数（来源：dispatch_env.yaml） |
| running_time_redundancy_ratio | 0.85 | 运行时间冗余系数（来源：dispatch_env.yaml） |
...
**冗余系数说明**：
- 停站时间冗余系数 0.8 表示：列车实际停站时间最短可为计划停站时间的80%
- 运行时间冗余系数 0.85 表示：列车区间运行时间最短可为计划运行时间的85%
